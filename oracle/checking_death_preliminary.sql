...
begin
  ...
  for r in (
    select i.id as msg_id, c.id as call_id
      from uszn.all_smev3_inc_messages i inner join uszn.all_smev3_calls c on i.orig_message_guid=c.message_guid
      where c.region_id=iRegionID and c.request_pdoc_id=iPDocID
      order by i.id desc)
  loop
    iMessageID := r.msg_id;
    iCallID    := r.call_id;
