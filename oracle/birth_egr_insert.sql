...
(select uszn.pkXMLImp.GetTagValueByName(fio.id, 'Счет', 0, 0)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведРодившемся'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='ПрСчет'
...
(select uszn.pkXMLImp.GetTagValueByName(birth.id, 'СНИЛС', 0, 0)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
...
(select uszn.ToDateISODef(uszn.pkXMLImp.GetTagValueByName(fio.id, 'ДатаДок', 0, 1))
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='УдЛичнФЛ'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_dul_data,

(select uszn.pkXMLImp.GetTagValueByName(adr.id, 'ОКТМО', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
  ...
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_adr_oktmo,
  
(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'КодРегион', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_adr_region_id,

(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'НаимРегион', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_adr_region,

(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'Город', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_adr_city,

(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'Район', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_adr_mo,

(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'НаселПункт', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as mama_adr_np,

(select uszn.pkXMLImp.GetTagValueByName(adr.id, 'АдрРФТекст', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведМать'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
...
(select uszn.pkXMLImp.GetTagValueByName(adr.id, 'ОКТМО', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведОтец'
       inner join uszn.r$_parsed_xml_data fio on fio.owner_id=birth.id and fio.name='АдрМЖ'
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as papa_adr_oktmo,
...
(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'КодРегион', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведОтец'
       ...
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as papa_adr_region_id,
...
(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'НаимРегион', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведОтец'
       ...
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as papa_adr_region,
...
(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'Город', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведОтец'
       ...
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as papa_adr_city,
...
(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'Район', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведОтец'
       ...
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as papa_adr_mo,
...
(select uszn.pkXMLImp.GetTagValueByName(adr_np.id, 'НаселПункт', 0, 1)
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведОтец'
       ...
       inner join uszn.r$_parsed_xml_data adr on adr.owner_id=fio.id and adr.name='АдрМЖРФ'
       inner join uszn.r$_parsed_xml_data adr_np on adr_np.owner_id=adr.id and adr_np.name='АдрКЛАДР'
  where sved.owner_id=reg.id and sved.name='ПрдСведРег') as papa_adr_np,
...
(select uszn.ToDateISODef(uszn.pkXMLImp.GetTagValueByName(birth.id, 'ДатаДок', 0, 1))
  from uszn.r$_parsed_xml_data sved inner join uszn.r$_parsed_xml_data birth on birth.owner_id=sved.id and birth.name='СведДокРод'
