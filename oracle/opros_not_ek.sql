...
(SELECT distinct
   ...
   WHERE t1.region_id in (58,59,60,61,62,63,64,65,66,67,68,69,70)
         and t1.service_date between trunc(trunc(sysdate, 'mm')-1, 'mm') and trunc(sysdate, 'mm')-1
         and t1.latitude is null
         ) a
  WHERE (upper(a.cso) LIKE '%ГБУ%' OR upper(a.cso) LIKE '%ГКУ%')
        AND (a.phone is not null or a.phone2 is not null)
		--AND uszn.pkPerson.GetPersonalReq(a.region_id, a.id,  4)=0