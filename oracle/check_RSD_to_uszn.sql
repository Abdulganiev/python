...
from uszn.temp$_kind_as_rsd t1
     inner join
     (select snils, max(to_date(DATA_REGISTRA)) as date_file from uszn.temp$_pfr622_C where DATA_REGISTRA> ADD_MONTHS(trunc(current_date, 'mm'), -1) group by snils) t5
  on t1.snils=t5.snils
     inner join
     (select region_id, people_id, asg_date, sum(amount) as amount from uszn.temp$_kind_as_rsd group by region_id, people_id, asg_date) t2
