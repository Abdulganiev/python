...
#**************************************************
name_log = 'birth_egisso'
mail = 'IVAbdulganiev@yanao.ru'

#**************************************************
def search_messages(curs):
    # curs = connect_oracle()
    with open('birth_egisso_find.sql', 'r', encoding='utf8') as f:
        sql = f.read()
    curs.execute(sql)
    df = curs.fetchall()
    # curs.close()
...
def processing(curs, region_id, id):
    with open('birth_egisso_check.sql', 'r', encoding='utf8') as f:
        sql = f.read()
    ...
    if cnt == 0:
        with open('birth_egisso_preliminary.sql', 'r', encoding='utf8') as f:
            sql = f.read()
        sql = sql.replace('{region_id}', f'{region_id}')
        sql = sql.replace('{id}', f'{id}')
        curs.execute(sql)

        with open('birth_egisso_insert.sql', 'r', encoding='utf8') as f:
            sql = f.read()
        curs.execute(sql)
        return 1
    return 0
...
#**************************************************        
def birth_egisso_cnt(curs):
    with open('birth_egisso_cnt.sql', 'r', encoding='utf8') as f:
        sql = f.read()
    curs.execute(sql)
...
try:
    dt = birth_egisso_cnt(curs)
except Exception as e:    
    text = f'произошла ошибка при вызове функции birth_egisso_cnt() - {e}'
    alarm_log(mail, name_log, text)    
