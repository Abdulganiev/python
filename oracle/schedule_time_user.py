...
#***************************************************************
name_log = 'schedule_time_user'
mail = 'IVAbdulganiev@yanao.ru'

writing_to_log_file(name_log, f'**************************************************************')
writing_to_log_file(name_log, f'Запуск schedule_time_user')

#***************************************************************
def user_iszn_drop():
    try:
        subprocess.call("python d:/python/schedule/user_iszn_drop.py", shell=True)
    except Exception as e:
        text = f'произошла ошибка при вызове функции user_iszn_drop() - {e}'
        writing_to_log_file(name_log, text)

try:
    writing_to_log_file(name_log, f'Запуск процесса user_iszn_drop')
    schedule.every(120).seconds.do(user_iszn_drop)
except Exception as e:
    text = f'Произошла ошибка при вызове функции user_iszn_drop - {e}'
    writing_to_log_file(name_log, text)
